...
import melee
import numpy as np
from melee.enums import ControllerType, Action, Menu
...
def build_console(
    ...
    kwargs = dict(
        ...
        # TODO: for now, only one worker saves replay, we have issues with replays having the same name when generated by dolphin
        save_replays=save_replays and slippi_port == 51441,
        fullscreen=False,
    )
...
class SSBM(PolarisEnv):
    ...
    def tag_replay_file(self) -> GameState:
        # we are about to generate a replay, use a filelock to ensure we are going to tag the right file
    ...
    def reset(
        ...
        if self.env_index == 0:
            for port, opt in options.items():
                print(opt.character, opt.tag, gamestate.players[port].character)
    ...
    def step(
        ...
        if done:
            ...
            self.slp_replay_manager.inject_bot_info(self.game_info["bot_a"], self.game_info["bot_b"])
