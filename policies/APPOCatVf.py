...
class APPOCatVf(ParametrisedPolicy):
    ...
    def __init__(
        ...
        if self.is_online:
            self.offline_policy = APPOCatVf(*args, online=False, **kwargs)
        else:
    ...
    def _train(
        ...
        with tf.GradientTape() as tape:
            with tf.device('/gpu:0'):
                (action_logits, _), all_vf_preds_cat = self.model(
                    input_batch
                )
                ...
                all_vf_preds = self.model.compute_predicted_values()
                ...
                batch_sigma = tf.math.sqrt(
                    ...
                    + tf.reduce_mean(tf.math.square(gvs))
                )
                #pg_advantages = clipped_rhos * (gpi - tf.stop_gradient(vf_preds))
                #normalised_pg_advantages = self.return_based_scaling.normalise(pg_advantages,
                ...
                critic_loss = tf.reduce_mean(tf.losses.categorical_crossentropy(
                    y_true=self.model.targets_to_probs(gvs),
                    y_pred=tf.boolean_mask(all_vf_preds_cat[:-1], mask),
                    from_logits=True,
                )) * self.policy_config.baseline_coeff
        ...
        return {
            ...
            "min_vtrace_returns": tf.reduce_min(gvs),
            "max_vtrace_returns": tf.reduce_max(gvs),
            "vf_mse": tf.reduce_mean(tf.math.square(gvs-vf_preds))
        }