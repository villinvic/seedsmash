...
from polaris.policies.utils.random_goal_exploration import RandomGoalExploration
...
class APPORandomGoals(ParametrisedPolicy):
    ...
    def __init__(
        ...
        if self.is_online:
            self.offline_policy = APPORandomGoals(*args, online=False, **kwargs)
        else:
        ...
        self.random_goal_exploration_module = RandomGoalExploration(
    ...
    def train(
        ...
        self.random_goal_exploration_module.randomise_goals(self.model, self.version)
    ...
    def _train(
        ...
        with tf.GradientTape() as tape:
            with tf.device('/gpu:0'):
                ...
                rewards = input_batch[SampleBatch.REWARD] + self.random_goal_exploration_module.compute_rewards(
                    model=self.model
                )[:-1] * self.policy_config.random_goal_weight
        ...
        metrics = {
            ...
            "batch_rewards": tf.reduce_mean(tf.math.abs(tf.boolean_mask(input_batch[SampleBatch.REWARD], mask))),
            "offline_to_online_kl": kl_offline_to_online
        }

        metrics.update(**self.random_goal_exploration_module.get_metrics())
