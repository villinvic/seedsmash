...
class IASAFC(BaseModel):
    ...
    def __init__(
        ...
        self.mlp_iasa_concat = tf.keras.layers.Concatenate(axis=-1, name="mlp_iasa_concat")
    ...
    def forward(self,
        ...
        xiasa = self.mlp_iasa_concat(
            [x, tf.cast(obs["binary"]["iasa1"], tf.float32)]
        )

        pi_out = self._pi_out(xiasa)
        self._values = tf.squeeze(self._value_out(xiasa))
