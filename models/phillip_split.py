...
class PhillipSplit(BaseModel):
    ...
    def __init__(
        ...
        super(PhillipSplit, self).__init__(
            name="PhillipSplit",
    ...
    def single_input(
        ...
        x = tf.concat(stage_embeds + opp_embeds + self_embeds, axis=-1)
        ...
        policy_embeds, core_next_state = self.core_rnn(
            core,
            core_state,
        )

        # oracle value function
        opp_embeds = self.opponent_embedder(obs, single_obs=True)
        value_inputs = tf.concat(stage_embeds + self_embeds + opp_embeds, axis=-1)
        value_embeds, value_next_state = self.core_rnn(
            value_inputs,
            value_state,
        )

        return policy_embeds, value_embeds, (opp_next_state, core_next_state, value_next_state)
    ...
    def batch_input(
        ...
        policy_embeds, _ = snt.static_unroll(
            self.core_rnn,
            input_sequence=core,
        ...
        # oracle value function
        opp_embeds = self.opponent_embedder(obs)
        value_inputs = tf.concat(stage_embeds + self_embeds + opp_embeds, axis=-1)
        value_embeds, _ = snt.static_unroll(
            self.core_rnn,
            input_sequence=value_inputs,
            initial_state=value_state,
            sequence_length=seq_lens
        )
